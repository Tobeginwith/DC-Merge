    ...
    for epoch in pbar:
        for i, (inputs, labels) in enumerate(train_loader):
            ...
            if (step+1) % training_config['eval_freq'] == 0:
                ...
                print(f'Steps {step+1}, Test Acc: {test_acc*100.0}, Test Loss: {test_loss}')
                print(f'Steps {step+1}, Val Acc: {val_acc*100.0}, Val Loss: {val_loss}')
                print(f'Steps {step+1}, Train Acc: {train_acc*100.0}, Train Loss: {train_loss}')
                ...
                if early_stopper.early_stop(performance_log, model, model_save_path): 
                    print(f"Early stopping at : {step+1}")
                    end = True             
                    break
...
def train_functional(training_config = None):
    ...
    model_save_dir = os.path.join(MODEL_SAVE_DIR, f"lora_rank{lc['r']}_b16")
    ...
    for dataset_name, loader_dict, class_vectors in tqdm(zip(dataset_names, data_loaders, all_clip_encodings)):
        ...
        save_path = os.path.join(
            model_save_dir, 
            f"{dataset_name}_lr_{training_config['lr']}_wd_{training_config['wd']}_max_steps_{training_config['max_steps']}_early_stopping_patience_{training_config['early_stopping_patience']}.pt"
        )
        print('\n\n\n')
...
if __name__ == "__main__":
    ...
    # CONFIG_NAME = '12vision_train'
    CONFIG_NAME = '16vision_train'
    ...
    train_functional(training_config)
